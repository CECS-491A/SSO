<template>
  <div class="reset">
    ...
    <div class="NewPassword" v-if="securityAnswersCorrect">
      {{passwordMessage}}
...
export default {
  ...
  created () {
    ...
    axios({
      ...
      .then(response => (this.securityQuestions = response.data),
        this.message = 'Enter your answers for the security questions')
  ...
  methods: {
    submitAnswers: function (e) {
      axios({
        method: 'POST',
        url: 'http://localhost:60461/api/reset/' + this.resetToken + '/checkanswers',
        ...
        .then(response => (this.securityAnswersCorrect = response.data))
        .catch(e => { this.errors.push(e) })
    },
    submitNewPassword: function (e) {
      axios({
        method: 'POST',
        url: 'http://localhost:60461/api/reset/' + this.resetToken + '/resetpassword',
        ...
        .then(response => (this.newPasswordSuccessful = response.data))
        .catch(e => { this.errors.push(e) })
    }
  }
}
